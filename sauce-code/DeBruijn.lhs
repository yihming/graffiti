...
finalProcess (S.Bop op t1 t2) = case op of
                                  S.IntAdd -> Bop Add (finalProcess t1) (finalProcess t2)
                                  S.IntSub -> Bop Sub (finalProcess t1) (finalProcess t2)
                                  S.IntMul -> Bop Mul (finalProcess t1) (finalProcess t2)
                                  S.IntDiv -> Bop Div (finalProcess t1) (finalProcess t2)
                                  S.IntNand-> Bop Nand (finalProcess t1) (finalProcess t2)
finalProcess (S.Bpr p t1 t2) = case p of
                                 S.IntEq -> Bpr Eq (finalProcess t1) (finalProcess t2)
                                 S.IntLt -> Bpr Lt (finalProcess t1) (finalProcess t2)                                  
{-                                  
...
toDeBruijn t = 
  ...
  where
    ...
    f (S.Bop op t1 t2, e) = S.Bop op (f(t1, e)) (f(t2, e))
    f (S.Bpr p t1 t2, e) = S.Bpr p (f(t1, e)) (f(t2, e))
    {-
