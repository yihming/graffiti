...
step state = case state of        
        ...
        (Apply:code, env, (Value v):(Value (CloFix [Close [Close code', Return], Fix] env')):s) -> Just(code', v:(CloFix [Close [Close code', Return], Fix] env'):env', (Code code):(Env env):s)
        ...
        (Fix:code, env, (Value (Clo code' env')):s) -> Just (code', (CloFix [Close code', Fix] env'):env, (Code code):(Env env):s)
        _ -> Nothing
loop :: State -> State
loop state =
        case step state of
                Just state'@((c,e,s))-> loop state'
                Nothing -> state
