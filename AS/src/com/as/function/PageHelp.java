...
public class PageHelp {
  ...
  public static int pagesize=5;			//每页显示记录数，系统统一的显示条数设置
  ...
  public int getRecordcount() { // 获取记录总条数
    return recordcount;
  }

  public void setRecordcount(int recordcount) { // 设置记录总条数
    PageHelp.recordcount = recordcount;
  
  }

  public int getPagesize() {//获取每页显示的记录条数
    return pagesize;
  }

  public void setPagesize(int pagesize) { // 设置每页显示的记录条数
    PageHelp.pagesize = pagesize;
  }

  public java.util.List getObjectlist() { // 获取显示内容
    return objectlist;
  }

  public void setObjectlist(java.util.List objectlist) {  // 设置显示内容
	 this.objectlist = objectlist;
  }
  ...
  public void setPagebar() {//显示首页、上一页、下一页、尾页，当前多少页、共多少页等的分页条
    ...
    if (PageHelp.currentpage <= 1) {//如果list表中记录数可以在一页中显示，则上一页，首页无链接
      ...
      strResult = "<a href='#' onClick='return changepage(1)'><font color='red'>首页</font></a> ";
      strResult += "<a href='#' onClick='return changepage("+(PageHelp.currentpage - 1)+")'><font color='red'>上一页</font><a>";
    }
    if (PageHelp.currentpage >= PageHelp.pagecount) {//如果当前页是显示的最后一页，则下一页，尾页无链接
      strResult += " 下一页 尾页";
    }else {//否则，只要记录不是最后一页都有链接实现点击分页
      strResult += "<a href='#' onClick='return changepage(" +(PageHelp.currentpage + 1) + ")'><font color='red'> 下一页</font><a> ";
      strResult += "<a href='#' onClick='return changepage(" + PageHelp.pagecount + ")'><font color='red'>尾页</font><a> ";
    }
  ...
  public String getPagebar() { // 获取分页条
	    return pagebar;
	  }
}
